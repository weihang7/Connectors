...
class ClientAuth:
    ...
    def __init__(self):
        ...
        _AUTH_REQ=Netatmo_cred.get("url")["_AUTH_REQ"]
        resp = getRequest(_AUTH_REQ, postParams)
...
class DeviceList:
    ...
    def __init__(self, authData):
        ...
        resp = getRequest(_DEVICELIST_REQ, postParams)
        #print "\nDeviceList: ",resp
    ...
    def getModuleData(self, module_id):
        """
    ...
    def getStationsData(self,station=None):
        """
        ...
        module_data={}
        for n in s['modules']:
            module_data.update(self.getModuleData(n))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not auth.clientId or not auth.clientSecret or not auth.username or not auth.password :
            stderr.write("Please Enter the Netatmo Clientid and ClientSecret")
            exit(1)
            
        devList = DeviceList(auth)         #Obtain the DEVICELIST
        netatmo_data= devList.getStationsData()     #Get the Stations data
        try:
            resp=devList.post_BD(netatmo_data)  #Send Data to the BuildingDepot
