...
class BD_connect:
    ...
    def _get_sensor_list(self,call_type):
    ...
    def check_sensor(self):
        ...
        '''
        sensor_list=str(self._get_sensor_list('all_sensors'))
    ...
    def _create_sensor_points(self):
    ...
    def rest_post(self,mac_id):
        ...
        '''
        response=''
        sensor_list=self._get_sensor_list(self.data['mac_id'])['data']
...
def get_json(data):
    ...
    '''
    info = BD_connect(data)
    #Check Valid Client id and Client key
    if 'Invalid credentials' not in info._get_oauth_token():
        ...
        if (set(check).issubset(info.data)):
            ...
            if not info.check_sensor():
                return info._create_sensor_points()
            else:
                return info.rest_post(info.data['mac_id'])
        else:
